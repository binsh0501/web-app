...
app.get('/', (req, res)=>{
    ...
    fs.readdir('page', (err, files)=>{
        ...
        fs.readFile(`page/${name}`, 'utf8', (err, data)=>{
            let control = `<a href="/create" class="linkStyle">create</a> <a href="/update?name=${name}" class="linkStyle">update</a>
            ...
            if(name === undefined){
                name = 'Solar System'
                data = '태양계는 항성인 태양과 그 중력에 이끌려 있는 주변 천체가 이루는 체계를 말한다'
                control = `<a href='/create' class="linkStyle">create</a>`
            }
